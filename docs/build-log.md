...
## Session 8 - 04/17/26 Cindy
# Pipeline Data Log — `run()`

**File:** `backend/scoring.py`

Updated the scoring variables in `scoring.py` so they now align with the variable names and structure used throughout the pipeline. This change improves consistency between the scoring logic and the pipeline outputs, helping prevent mismatches when data is passed between modules. The goal was to make sure `scoring.py` reads and processes the same variables generated by the pipeline without needing separate conversions or manual adjustments.
...
**What we built:**
- T014 — `cache.py` with `get_cached_players()` and `set_cached_players()` (1-hour expiry using `datetime` + `timedelta`)
- Updated `PlayerStats` to add `fg_pct` and `ft_pct` fields
- Simplified `calculate_sustainability` to use percentages directly instead of recomputing from raw counts
- Added `adapt_pipeline_row()` — adapter function translating pipeline column names to `PlayerStats` field names

**Key decisions:**
- Cache expiry = 1 hour (balance between data freshness and API rate limits)
- Adapter pattern at the pipeline → scoring boundary (cleanly separates the two concerns)
- Build for `season_averages` data first; `game_logs` deferred to V2 for trend signals
- Scoped project as V1 (waiver wire recommender) → V2 (game_logs trends) → V3 (full fantasy platform)

**Next:** T015 — Wire cache into main.py

---

## Session 9 — 05/04/26 Justin
**Starting at:** T015 — Wire cache into main.py (planned)

**What we built / fixed:**
- Recovered Session 8 work after sync confusion — git pull resolved a "rejected push" caused by pushing to main while I was offline
- Refactored `calculate_minutes_trend` to use `len(recent_minutes) // 2` instead of hardcoded `[:2]` and `[2:]` — function now adapts to any `RECENT_GAMES_WINDOW` value
- Discovered data contract mismatch between scoring engine and pipeline by reading source directly (`pipeline/ingest.py` line 481, `constants.py` line 52)
- Identified that `_drop_unused()` strips `fg_pct`, `ft_pct`, `games_played`, and `turnover` from `season_averages` output — these are required by the scoring engine
- Sent a specific request with file:line references to remove those four from `UNUSED_COLUMNS["season_averages"]`

**Key decisions:**
- Cache-aside pattern chosen for V1: Option A (startup warmup) + Option B (lazy refresh on expiry). Option C (background scheduler) deferred to V2.
- Pause T015 wiring until pipeline data contract is fixed — wiring the cache without correct data is putting the cart before the horse
- Read the code first, ask second — checked pipeline directly via `grep` instead of waiting for him to explain

**Lessons learned:**
- `git pull` before AND after every session prevents sync issues
- Empty `cache.py` in a downloaded zip ≠ work is lost; check git status first
- "Magic numbers" (hardcoded values) are a code smell — replace with computed values or named constants
- Data contract mismatches are a top cause of integration bugs in team projects
